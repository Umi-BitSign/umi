...
that any activation gate in Section 14 has passed. The addendum expires by its
fixed block schedule. It narrowly overrides Section 10.3 only to permit the service
row while `translation_weights_active` remains false and to allow pre-reveal
publication of the already-public binary eligibility manifest and expected row.
...
- **Publisher control group**: publisher hotkeys under common beneficial ownership
  or control, common administration or operational decision authority, common
  custody or recovery authority over publisher wallets or collateral, common
  privileged or non-public access to plaintext ground truth or sealing material
  before the applicable reveal, or a funding arrangement that confers material
  influence over any of those matters. Exposure caps apply to the group, not
  merely to its hotkeys.
...
| Who bears single-use challenge cost? | The protocol assigns no publisher emission. Each publisher is accountable for financing and delivering its candidate batches, including work that retires unselected. An independent publisher MAY receive disclosed arm's-length external compensation that passes Section 4.3's control test; direct collateral funding or influential financing affects its control-group classification. The founding control group currently bears its own cost. Only benchmark eligibility is spent after reveal; the underlying data is not destroyed, although any later use still depends on its consent class. |
...
coldkeys, and declared control-group IDs in the scoring policy. Hotkeys that meet
the publisher-control-group definition in Section 1.1 MUST share one group.
Standard arm's-length compensation for completed goods or services does not by
itself establish common funding or common control when all of these conditions
hold:

- the amount, term, and objective payment events are fixed before the work and are
  not contingent on miner outputs, scores, weights, emissions, or batch selection;
- the payer has no beneficial ownership, wallet or collateral custody,
  administrative authority, privileged pre-reveal access to plaintext ground
  truth or sealing material, right to select or replace contributors or reviewers,
  reference-approval right, or direction over prompt creation, review, sealing,
  publication, or incident decisions; and
- the provider controls its own personnel, systems, credentials, wallets,
  collateral, and protocol decisions.

Ordinary contractual rights to enforce applicable law, confidentiality, uniform
public policy-pinned protocol and security requirements, objective delivery
criteria, and payment do not alone establish control. Those rights MUST NOT include
publisher-specific approval, selection, discretion, access, or direction over the
protected functions above. Direct collateral funding, shared administrators or
reviewer operations, financing outside the fixed contract that can direct protocol
decisions, score-linked revenue sharing, or any access or decision right named
above remains evidence of common control. Every funding and service relationship
MUST be disclosed and assessed on its actual operation. Calling a payment an
invoice, grant, or standard rate does not create a safe harbor.

Before its first window in which it is represented as separate from another group,
and no later than its first qualifying soak window, every declared publisher
control group MUST publish one signed control disclosure. The public object
contains no contributor or reviewer identity, contract text, rate, or private
system detail. It uses this logical schema:

```json
{
  "schema": "umi-publisher-control-disclosure/1",
  "statement": {
    ...
    "classification": "separate_control_group",
    "relationships": [
      {
        "relationship_id": "hex-encoded-32-bytes",
        "related_party_account": "ss58-public-attestation-account",
        "relationship_kinds": ["service_compensation"],
        "private_evidence_sha256": "hex-encoded-sha256",
        "facts": {
          "terms_fixed_before_work": true,
          "miner_output_contingent": false,
          "score_contingent": false,
          "weight_contingent": false,
          "emission_contingent": false,
          "batch_selection_contingent": false,
          "common_beneficial_ownership": false,
          "common_administration_or_decision_authority": false,
          "wallet_or_collateral_custody": false,
          "direct_collateral_funding": false,
          "privileged_pre_reveal_ground_truth_access": false,
          "protected_function_direction": false,
          "uniform_objective_requirements_only": true,
          "provider_controls_operations": true
        }
      }
    ],
    "material_change_commitment": true
  },
  "signatures": [
    {
      "signer_account": "ss58-administrator-or-required-payer-account",
      "scheme": "sr25519",
      "signature": "0x..."
    }
  ]
...
`relationship_kinds` uses the version 0.1 enumeration `ownership`,
`administration`, `service_compensation`, `other_funding`, `wallet_or_collateral`,
`infrastructure`, `ground_truth`, and `other`, sorted in that order without
duplicates. Relationships are sorted by raw `relationship_id`; duplicate
relationship IDs are invalid. Publisher hotkeys and signatures are sorted by
decoded account. The statement includes every known
owner, administrator, funder, wallet or collateral custodian, ground-truth
operator, shared infrastructure administrator, and non-commodity service
relationship relevant to the classification. `private_evidence_sha256` binds the
restricted contract or assessment record without publishing it.

The signature list contains exactly the administrator and every distinct payer
account named under `service_compensation` or `other_funding`, without duplicates.
Each signs the same raw digest:

```text
publisher_control_disclosure_digest = SHA256(
  "umi-publisher-control-disclosure-v1\0" || RFC8785(statement)
)
```

Signatures follow Section 6.3. The complete signed object's canonical SHA-256 is
the `control_disclosure_sha256` in the group's capacity statement. The disclosure
statement and capacity statement MUST have byte-identical `control_group_id`,
`administrator`, `publisher_hotkeys`, and `scoring_policy_hash` values. Both bind
the shadow policy used for the soak. The activation-equivalent policy preserves
that pair only when the capacity statement's `activation_equivalence_digest`
matches the digest recomputed from both policies under Section 14; the changed
activated-policy hash does not by itself require replacement. Any disclosure
replacement changes `control_disclosure_sha256` and requires a replacement,
re-signed capacity statement. The bytes MUST be public before the soak announcement
block and remain available with every soak bundle. Signatures authenticate the
disclosed assertions; they do not prove that an off-chain fact is complete or
true.

A material change to ownership, administration, funding terms, custody, privileged
pre-reveal access, protected-function authority, or any relationship named in the
statement invalidates each not-yet-revealed window that relied on the prior
classification and restarts the soak. When the change occurs or is discovered, the
group MUST immediately publish a signed material-change notice naming the prior
disclosure hash, effective or discovery block, and affected relationship kinds,
and MUST pause pool proposals. Validators that receive the notice MUST pause
publisher intake and UMI weight submission for every affected window. Every
validator MUST check the public notice location at pool close and again before
weight build; any affected window still awaiting reveal is void. The group MUST
publish and sign a replacement disclosure and capacity statement before another
pool proposal. A resulting
registry or control-group change also requires a new policy hash and activation
block under the rule below. Failure to disclose remains an off-chain residual risk.

Adding an entry, removing an entry, or changing a group requires a new policy hash
and activation block. The new registry mapping does not retroactively remap an open
selection window, but this does not preserve a window invalidated by a material
factual change under the preceding paragraph. A new identity is a new governance
decision backed by fresh collateral; the protocol does not infer
...
protocol-assigned publisher emission in version 0.1. Each publisher is accountable
for financing and delivering every batch it proposes, including an unselected
batch that retires unused. It MAY fund that work internally or receive disclosed
fixed arm's-length compensation under the control test above. External payment
creates no protocol entitlement, reimbursement claim, or exception to retirement.
...
- Control-disclosure signatures authenticate assertions by the named accounts;
  they cannot prove that an off-chain relationship, access path, or change was
  disclosed completely or truthfully.
- A common arm's-length payer can create correlated funding and supply risk even
  when it has no publisher-control rights. Capacity statements cannot prove that
  the payer or providers will remain available through the declared runway.
...
- The current owned finality client verifies GRANDPA internally but emits a
  ...
  explicitly trusts the governed, policy-pinned Finney
  `grandpaWarpSyncCheckpoint` and its authority set as a weak-subjectivity input,
  then verifies peer-supplied warp and GRANDPA data from that point forward.
...
| Publisher control disclosure | `umi-publisher-control-disclosure/1` |
...
A breaking request, pool, mirror, video-delivery, ground-truth, timelock, registry,
or scoring change MUST increment its version. A scoring change MUST declare an
activation block and MUST NOT alter any selection window opened under an earlier
policy hash.
...
Miners MUST expose `POST /v1/translate` over HTTPS at their announced serving
endpoint. The certificate MUST be valid for the exact IP address in the finalized
serving record under the validator's system trust store. Requests MUST use
`btauth/1`. Deployments with several server processes MUST share replay state.
...
Each request MUST carry exactly one `X-UMI-Body-SHA256` header containing the
lowercase hexadecimal SHA-256 of the exact body. This is the body-hash line already
covered by the `btauth/1` signature, not a second authentication scheme. Before
reading the body, the miner verifies the canonical authentication-header set,
receiver, freshness, signature, and policy validator account against that declared
digest. It then reads the body under the byte and time ceilings, checks the actual
digest, and atomically consumes the nonce before parsing the request. A failed
signature or body-digest check consumes no nonce. Duplicate authentication or body-
digest headers are malformed.
...
Protocol and artifact requests MUST advertise `Accept-Encoding: identity`.
Receivers MUST reject a response carrying any non-identity `Content-Encoding` and
MUST apply byte ceilings to the undecoded response stream. Compression at a storage
or transport layer outside the signed HTTP representation does not change this
rule.

The policy pins `btauth_max_age_seconds` and `btauth_allowed_skew_seconds`. The
maximum age MUST be at least `issue_allowance_seconds`, because the exact initial
signed request is committed and finalized before transmission. At launch these
values are 360 seconds and 2 seconds. Miners MUST retain replay nonces for at least
their sum, and every retry still uses a fresh nonce. Expanding the freshness window
does not permit replay of an already accepted signature. Durable replay state MUST
be keyed by decoded policy-validator `AccountId32`, reject accounts outside that
registry before insertion, and enforce per-validator, total-row, and database-byte
ceilings derived from the maximum assignment and retry counts. Reaching a ceiling
fails closed and MUST NOT evict a still-fresh nonce.
...
all video-fetch traffic, and the response it sends. A miner isolates unverified
video declarations, pending fetches, and failed-fetch budgets by authenticated
issuing-validator hotkey. One validator therefore cannot spend another validator's
fetch budget or bind a public video digest to false metadata. A hash-verified body
may enter the miner's shared cache only after its size and digest match; only that
verified body may be reused across validator partitions. The miner's aggregate
assignment, wire, and retained-byte ceilings still cover all partitions. Each actor
enforces its applicable counter before the next read or write. Shared validator
fetches count once at the validator window level rather than once for every miner
assignment.

A miner implementation MUST reserve one nonqueueing request-work slot and one
runnable inference slot for every validator in the active policy registry. Its
total enforced model concurrency MUST be at least that registry size. An isolated
model process MUST bind the same minimum, the policy hash, and its model revision
to an owner-private startup record that the protocol process checks before each
request. These checks do not prove that a model framework or accelerator executes
work concurrently internally; the activation soak measures that residual
operating risk.
...
- The URL MUST be a credential-free HTTPS delivery URL issued after batch
  selection. Its origin MUST appear in the mirror rule's sorted
  `delivery_origins`, which are disjoint from the authenticated retrieval origins.
  The URL MUST remain valid through `response_close_round`, expire within the
  policy-pinned delivery grace after it, and use exactly
  `/v1/umi/deliveries/<token>`, where `token` is the canonical unpadded base64url
  encoding of the 24-byte value derived below. Credentials, queries, fragments,
  percent encoding, additional path segments, and any label, prompt, signer name,
  or task answer are forbidden. An authenticated mirror object URL MUST NOT enter
  a miner request.
- Before the first issuance attempt, the validator MUST generate and durably store
  one 256-bit cryptographically random `delivery_token_seed` for the window. The
  seed enters the canonical authenticated video-delivery issuance request and MUST
  be reused after a retry or restart. For each selected item, the delivery service
  MUST use the first 24 bytes of this digest as the URL token, encoded as canonical
  unpadded base64url:
...
```
  ...
  Hash fields in this formula are raw 32-byte values. A service-chosen token or a
  response that does not reproduce this token is invalid and cannot be cached as a
  successful issuance.
...
`normalized_script_sha256` is not a hash of the publisher's raw prompt bytes. For
ordinary, canary-actual, and canary-reserved scripts alike, the publisher applies
the complete Section 9.1 text normalization and hashes the normalized UTF-8 bytes:

```text
normalized_script_sha256 = SHA256(UTF8(normalize_text_section_9_1(script)))
```

The JSON field carries the 32-byte result as lowercase hexadecimal. A script whose
normalized form is empty is ineligible.
...
policy also defines the content-addressed mirror discovery rule. Its canonical
bytes contain sorted authenticated retrieval origins, disjoint sorted public
`delivery_origins`, and the fixed authenticated post-selection delivery-issuance
endpoint. Publisher and validator registry changes require a new policy hash and
activation block.
...
ciphertexts. After selection, it retrieves every selected video, then sends one
canonical authenticated request to the mirror rule's fixed delivery-issuance
endpoint before preparing miner assignments. The request binds the window, batch,
challenge, video hash, and size. The canonical response binds the same fields and
provides one credential-free HTTPS URL and expiry per selected item. The validator
accepts only URLs under `delivery_origins` whose expiry is at least the response
close time and at most the end of the delivery grace. The exact discovery rule,
request, response, and non-secret exchange evidence enter the audit bundle and are
replayed before accepting the assignments. Every publisher, availability signer,
validator, and miner fetch streams
...
void. The validator proves that ordering with the closing header's hash-chained
local acceptance receipt. Its timestamp is evidence of that validator's local
clock, not chain- or GRANDPA-certified time, and cannot constrain a dishonest
validator clock.
...
A validator MUST obtain finalized headers through the policy-pinned owned-finality
profile, independently of the RPC provider used to retrieve block bodies or storage
proofs. The version 0.1 implementation embeds smoldot, connects directly to Finney
peers, and accepts no provider RPC URL for finality. Its public evidence is labeled
`verifier_attested_finality` and `offline_finality_proof: false`: smoldot verifies
warp and GRANDPA data internally, but its public wrapper does not export the
portable justification bytes needed for an auditor to verify finality from the
bundle alone. The bundle therefore pins and records the reviewed source, lockfile,
fixture set, release binary, chain specification, genesis hash, bootstrap header,
and hash-chained attestation transcript. It MUST NOT relabel that transcript as a
self-contained GRANDPA proof. The transcript and local acceptance receipts are
unsigned; their hashes detect alteration relative to a retained run but do not
prevent creation of a synthetic offline chain. Activation evidence MUST be
captured directly from an invocation of the exact policy-pinned sidecar over its
peer-to-peer path. A caller-supplied transcript is replay material only.

Version 0.1 accepts only a governed `grandpa_warp_sync_checkpoint` bootstrap. The
policy pins the complete official Finney chain-specification bytes, their subtensor
source revision, genesis hash, and the nonzero checkpoint number and hash. The
observer independently decodes the checkpoint header and GRANDPA authority set,
constructs one deterministic smoldot database, and requires a typed startup receipt
showing that this exact checkpoint was selected. Every emitted attestation binds
`bootstrap_source: grandpa_checkpoint`, `bootstrap_selected: true`, and the first
verified finalized head. Both that startup head and the first emitted header MUST
advance beyond the checkpoint. Genesis fallback, legacy `lightSyncState`, a locally
chosen checkpoint, or an ambiguous specification is non-conforming.

The checkpoint and its authority set are an explicit weak-subjectivity assumption
and can change only with a new policy hash. They are not proof that an RPC provider
reported finality honestly. From the selected checkpoint onward, the owned client
verifies peer-supplied warp and GRANDPA data without accepting a provider RPC URL.
Provider-supplied storage values become authoritative only after the independent
LayoutV1 proof verifier binds their raw `StorageProof` nodes and claimed values to
the attested header's state root.

When a verified header enters the validator's durable finality store, the store
captures a Unix-millisecond local acceptance time in the same transaction and
binds it, the header, the attestation digest, and the preceding receipt into an
append-only acceptance digest. Pool-close evidence includes the canonical receipt
for the closing header and requires its local acceptance time to precede publication
of the selection pulse. This is reproducible evidence of the validator's own event
ordering, not a GRANDPA-certified time or a defense against a dishonest validator
clock.
...
Each observation records its block number and hash, the exact queried value, and
any applicable event or extrinsic reference so another operator can repeat the
read at the named block. Bootstrap evidence declares
`storage_proofs_verified: false` and retains the selected chain provider as a
...
sunset checkpoint records the finalized flag. A still-enabled flag is a public
unsupported-emission incident; it never extends the bootstrap or authorizes
another service row. A future governed mechanism requires a separately published
superseding amendment and does not alter this bootstrap policy's sunset
conditions.
...
Except when executing the exact temporary `bootstrap_service_binary` addendum,
when the active policy has `translation_weights_active: false`, the validator stops
the chain path here. It records the projected translation row and a finalized event
scan proving that its hotkey emitted no UMI translation-weight commit for the
window, waits through the derived `weight_commit_close_block`, and publishes a
`calibration_no_weight` bundle. It MAY run the pinned ciphertext builder locally
for fixture coverage but MUST NOT sign or broadcast the resulting translation call.
The remaining submission and terminal-state requirements in this section apply to
an activated translation policy and, where the addendum expressly incorporates
them, to its separately labeled service row.
...
1. **Pool and selection:** window index, announcement and closing blocks, derived
   rounds, policy-declared finality evidence, pool and public batch manifests,
   closing-block proofs,
   availability certificate and certified bodies, applicable signed publisher
   control disclosures and, for a soak window, capacity statements for every
   selected source; selection pulse and trace; the exact mirror-discovery rule and
   canonical video-delivery issuance request, response, and evidence; every bounded
   successful, failed, or interrupted retrieval and issuance attempt; exactly
   reconciled observed and ceiling-accounted byte totals; encrypted ground-truth
   objects; previous and resulting spent roots; and previous and resulting
   publisher-fault roots with all sorted deltas.
...
section. Outside that exact addendum, `translation_weights_active: false` continues
to prohibit every UMI weight submission.
...
- exactly three active challenge-publisher hotkeys across exactly three
  ...
  relationships disclosed through valid public
  `umi-publisher-control-disclosure/1` objects whose hashes match the signed
  capacity statements, every hotkey meeting the locked-collateral requirement, and
  the signed runway statements plus realized soak evidence showing that any two
...
| Signed-request maximum age and allowed clock skew (`btauth_max_age_seconds`, `btauth_allowed_skew_seconds`) | 360 seconds and 2 seconds |
...
The initial five-minute issue allowance and five-minute response window are calibration
...
- obtains canonical short-lived miner delivery URLs after selection without
  exposing authenticated mirror source URLs;
...
Except for the exact temporary `bootstrap_service_binary` path incorporated in
...
- publishes the signed control disclosure before a qualifying soak, immediately
  reports any material relationship change, pauses proposals, and replaces both
  the disclosure and capacity statement before another proposal;
