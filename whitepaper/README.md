...
1. **Pool and selection:** window index, announcement and closing blocks, derived
   ...
   availability certificate and certified bodies, selection pulse and trace, the
   exact mirror-discovery rule and canonical video-delivery issuance request,
   response, and evidence; every bounded successful, failed, or interrupted
   retrieval and issuance attempt; exactly reconciled observed and ceiling-accounted
   byte totals; encrypted ground-truth objects; previous and resulting spent roots;
   and previous and resulting publisher-fault roots with all sorted deltas.
