...
\item
  \textbf{Publisher control group}: publisher hotkeys under common beneficial ownership
...
Who bears single-use challenge cost? & The protocol assigns no publisher emission. Each publisher is accountable for financing and delivering its candidate batches, including work that retires unselected. An independent publisher MAY receive disclosed arm\textquotesingle s-length external compensation that passes Section 4.3\textquotesingle s control test; direct collateral funding or influential financing affects its control-group classification. The founding control group currently bears its own cost. Only benchmark eligibility is spent after reveal; the underlying data is not destroyed, although any later use still depends on its consent class. \\
...
Standard arm\textquotesingle s-length compensation for completed goods or services does not by
...
\path|relationship_kinds| uses the version 0.1 enumeration \path|ownership|,
\path|administration|, \path|service_compensation|, \path|other_funding|, \path|wallet_or_collateral|,
\path|infrastructure|, \path|ground_truth|, and \path|other|, sorted in that order without
duplicates. Relationships are sorted by raw \path|relationship_id|; duplicate
...
relationship relevant to the classification. \path|private_evidence_sha256| binds the
...
account named under \path|service_compensation| or \path|other_funding|, without duplicates.
...
Signatures follow Section 6.3. The complete signed object\textquotesingle s canonical SHA-256 is
the \path|control_disclosure_sha256| in the group\textquotesingle s capacity statement. The disclosure
statement and capacity statement MUST have byte-identical \path|control_group_id|,
\path|administrator|, \path|publisher_hotkeys|, and \path|scoring_policy_hash| values. Both bind
the shadow policy used for the soak. The activation-equivalent policy preserves
that pair only when the capacity statement\textquotesingle s \path|activation_equivalence_digest|
...
replacement changes \path|control_disclosure_sha256| and requires a replacement,
...
fixed arm\textquotesingle s-length compensation under the control test above. External payment
...
\item
  A common arm\textquotesingle s-length payer can create correlated funding and supply risk even
...
\item
  ...
  \path|grandpaWarpSyncCheckpoint| and its authority set as a weak-subjectivity input,
...
Mirror discovery schema & \path|umi-mirror-discovery/1| \\
Video-delivery issuance schemas & \path|umi-video-delivery-issuance-request/1|, \path|umi-video-delivery-issuance-response/1|, and \path|umi-video-delivery-issuance-evidence/1| \\
Availability certificate & \path|umi-availability/1| \\
Publisher capacity schema & \path|umi-publisher-capacity/1| \\
Publisher control disclosure & \path|umi-publisher-control-disclosure/1| \\
...
Activation-drill schema & \path|umi-activation-drill/1| \\
...
Miners MUST expose \path|POST /v1/translate| over HTTPS at their announced serving
endpoint. The certificate MUST be valid for the exact IP address in the finalized
serving record under the validator\textquotesingle s system trust store. Requests MUST use
\path|btauth/1|. Deployments with several server processes MUST share replay state.
...
Each request MUST carry exactly one \path|X-UMI-Body-SHA256| header containing the
lowercase hexadecimal SHA-256 of the exact body. This is the body-hash line already
covered by the \path|btauth/1| signature, not a second authentication scheme. Before
...
Protocol and artifact requests MUST advertise \path|Accept-Encoding: identity|.
Receivers MUST reject a response carrying any non-identity \path|Content-Encoding| and
...
The policy pins \path|btauth_max_age_seconds| and \path|btauth_allowed_skew_seconds|. The
maximum age MUST be at least \path|issue_allowance_seconds|, because the exact initial
...
be keyed by decoded policy-validator \path|AccountId32|, reject accounts outside that
...
issuing-validator hotkey. One validator therefore cannot spend another validator\textquotesingle s
fetch budget or bind a public video digest to false metadata. A hash-verified body
may enter the miner\textquotesingle s shared cache only after its size and digest match; only that
verified body may be reused across validator partitions. The miner\textquotesingle s aggregate
...
\item
  ...
  selection. Its origin MUST appear in the mirror rule\textquotesingle s sorted
  \path|delivery_origins|, which are disjoint from the authenticated retrieval origins.
  ...
  \path|/v1/umi/deliveries/<token>|, where \path|token| is the canonical unpadded base64url
...
\item
  ...
  one 256-bit cryptographically random \path|delivery_token_seed| for the window. The
...
\path|normalized_script_sha256| is not a hash of the publisher\textquotesingle s raw prompt bytes. For
...
\path|delivery_origins|, and the fixed authenticated post-selection delivery-issuance
...
canonical authenticated request to the mirror rule\textquotesingle s fixed delivery-issuance
...
accepts only URLs under \path|delivery_origins| whose expiry is at least the response
...
the two reserved canary scripts. \path|first-disclosure time| is the instant the first
external access authorization becomes valid; \path|disclosure-evidence digest| binds
...
void. The validator proves that ordering with the closing header\textquotesingle s hash-chained
local acceptance receipt. Its timestamp is evidence of that validator\textquotesingle s local
...
count exceeds \path|target_count|. In that case, validators normally hold overlapping
...
diversity. Let \path|w_v| and \path|w_u| be two validators\textquotesingle{} normalized
...
attempt. The publisher\textquotesingle s private operating evidence MUST record quarantined
inventory, including attempts that never became \path|anchored-eligible|.
...
distributions from SN78 showing at least \path|2 * M_gate| miners sustaining positive
...
\item
  \textbf{Conditional distinct sampling.} Batch selection is common, while miner
  ...
  the copier\textquotesingle s declared panel. When every candidate fits in the panel, membership
...
validator cannot present a peer\textquotesingle s responses as its own conforming work. A validator
...
explicitly local to \path|v| and must be reproducible from that validator\textquotesingle s bundle.

The source-monitor bootstrap uses the policy-pinned \path|umi-source-bootstrap/1|
profile. Rows on each side of each stratum are grouped by the bundle\textquotesingle s opaque
...
publisher sources, \path|source_id32| is the publisher account; for control-group
...
\path|source_kind_byte| is \path|0x00| for a publisher and \path|0x01| for a control group. For
this seed, \path|window_id| is the selection window at which the rolling source monitor
is evaluated. For
zero-based replicate \path|b|, side byte \path|z| (\path|0x00| for the source and \path|0x01| for the
outside set), zero-based stratum index \path|k| in Section 7.2 table order, draw index
\path|d|, and sorted signer-cluster count \path|m|, the selected cluster index is:
...
\path|U64BE_prefix| interprets the first eight digest bytes as an unsigned big-endian
integer, and \path|U8(k)| is the single unsigned byte for \path|k|.
...
the aggregate effects in ascending order. For confidence \path|c| and \path|B| replicates,
the one-sided lower endpoint is the value at one-based rank
\path|max(1, ceil((1 - c) * B))|. Missing cluster IDs, a missing stratum component, or
...
\path|verifier_attested_finality| and \path|offline_finality_proof: false|: smoldot verifies
...
Version 0.1 accepts only a governed \path|grandpa_warp_sync_checkpoint| bootstrap. The
...
\path|bootstrap_source: grandpa_checkpoint|, \path|bootstrap_selected: true|, and the first
verified finalized head. Both that startup head and the first emitted header MUST
advance beyond the checkpoint. Genesis fallback, legacy \path|lightSyncState|, a locally
...
LayoutV1 proof verifier binds their raw \path|StorageProof| nodes and claimed values to
the attested header\textquotesingle s state root.

When a verified header enters the validator\textquotesingle s durable finality store, the store
...
of the selection pulse. This is reproducible evidence of the validator\textquotesingle s own event
...
count and \path|miner_panel_size| to be at least the live \path|MinAllowedWeights|. This is a
runtime safety floor. Section 14 applies the stricter \path|2 * M_gate| activation test.

\subsection{Weight submission}\label{103-weight-submission}

The temporary \path|bootstrap_service_binary| documents, consisting of the
\href{../docs/BOOTSTRAP_WEIGHT_ADDENDUM.md}{original policy addendum} and its
\href{../docs/SHARED_VALIDATOR_BOOTSTRAP_SUPERSESSION.md}{shared-validator supersession},
are the
only exception to this section\textquotesingle s prohibition on a weight call while
\path|translation_weights_active| is false. They may authorize only the addendum\textquotesingle s exact
...
local configuration binds that validator\textquotesingle s hotkey. The frozen eligibility policy
...
\path|translation_weights_active: false|.

For this bootstrap only, the addendum replaces this whitepaper\textquotesingle s portable
...
\path|storage_proofs_verified: false| and retains the selected chain provider as a
...
validator\textquotesingle s permit, exact stored row, and \path|LastUpdate| against the public signed
manifest and lease. The validator\textquotesingle s manifest commitment remains independently
auditable on chain.
Translation weights still require the chain-native commit-reveal path and all
ordinary evidence-release rules.

At the addendum\textquotesingle s commit stop, its coordinator MUST ask the authorized Const/root
operator to disable SN78 \path|subnet_emission_enabled| by the fixed hard sunset. The
...
superseding amendment and does not alter this bootstrap policy\textquotesingle s sunset
conditions.
...
Except when executing the exact temporary \path|bootstrap_service_binary| addendum,
...
window, waits through the derived \path|weight_commit_close_block|, and publishes a
\path|calibration_no_weight| bundle. It MAY run the pinned ciphertext builder locally
...
\href{../docs/BOOTSTRAP_WEIGHT_ADDENDUM.md}{\path|bootstrap_service_binary| policy} and its
\href{../docs/SHARED_VALIDATOR_BOOTSTRAP_SUPERSESSION.md}{shared-validator supersession}, which
...
section. Outside that exact addendum, \path|translation_weights_active: false| continues
...
Let \path|M_gate| be the greatest finalized \path|MinAllowedWeights| value in every logical
...
\path|MinAllowedWeights| remains a live chain input, not an owner-selected scoring-policy
...
Activation drills use one public \path|umi-activation-drill/1| report per drill. Its
logical schema is:
...
using independently implemented drivers sign the same \path|result_sha256|. Replayer
...
For each replayer, let \path|replayer_attestation| be the RFC 8785 object containing its
\path|administrator|, \path|implementation_sha256|, \path|result_sha256|, and \path|scheme| fields.
...
\path|result_sha256| is decoded to its raw 32 bytes in the signature formula. A missing
...
\bottomrule\noalign{}
\endlastfoot
\path|ground-truth-sealing| & Pre-reveal decryption is rejected; the declared round opens exactly the committed plaintext & none \\
\path|late-response| & The affected assignment remains in the denominator with score zero & \path|late| \\
\path|early-reveal| & The complete selection window is void & \path|ground_truth_early_reveal| \\
\path|mirror-loss| & Retrieval continues from a certified mirror without changing the candidate pool & none \\
\path|certificate-breach| & The affected validator skips the complete window and publishes an incident bundle & \path|certificate_breach| \\
\path|one-validator-loss| & The launch-minimum certificate with the remaining three valid signers still verifies & none \\
\path|availability-equivocation| & Conflicting quorum certificates make the window void & \path|availability_equivocation| \\
\path|malformed-pool| & The malformed anchored pool contributes no candidate & \path|malformed_pool| \\
\path|anchor-replacement| & The retained closing-block proof remains authoritative and a later replacement is ignored & none \\
\path|assignment-anchor| & The affected validator skips before issuance & \path|assignment_anchor_invalid| \\
\path|request-anchor| & The affected validator skips the window & \path|request_anchor_invalid| \\
\path|response-anchor| & The affected validator skips the window & \path|response_anchor_invalid| \\
\path|response-copy| & A response bound to another validator scores zero for the copied assignment & \path|response_validator_binding_mismatch| \\
\path|spent-replay| & A prior spent hit is ineligible, or a script hit learned after reveal makes the selected window void & \path|spent_replay| \\
\path|publisher-fault-replay| & Independent replay produces the same fault root, strike count, and cooldown state & none \\
\path|canary-hit| & The window is void, new active-policy submissions pause, and the hit appears in the incident bundle & \path|canary_hit| \\
\path|publisher-alert| & The exact source-conditioned interval produces public shadow telemetry and no score mutation & \path|publisher_divergence_alert| \\
\path|UID-reassignment| & The pending weight result terminates as failed & \path|uid_reassignment| \\
\path|accepted-but-unapplied-weight| & The accepted commit terminates as failed after exact-key removal & \path|accepted_but_unapplied_weight| \\
\path|duplicate-weight-commit| & The duplicate event makes the submission fail and pauses ordinary commits & \path|duplicate_weight_commit| \\
\path|epoch-pull-forward| & The early or differently keyed submission is rejected as the window result & \path|weight_epoch_pull_forward| \\
\path|epoch-deferral| & The late or differently keyed submission is rejected as the window result & \path|weight_epoch_deferral| \\
\path|reveal-period-mutation| & The entry remains tracked through removal, terminates as failed, and new submissions pause & \path|reveal_period_mutation| \\
\path|stale-row| & Ordinary commits remain paused until the prior row is inactive or a controlled recovery applies & \path|previous_row_still_active| \\
\path|legacy-row-cutover| & Activation remains closed while any legacy entry is unresolved or legacy row remains active & \path|legacy_weight_state_active| \\
\path|drand-profile-mismatch| & Startup fails closed before a window is processed & \path|drand_profile_mismatch| \\
\path|MEV-Shield-expiry| & Expired transport is not reported as successful inner-call execution & \path|mev_shield_expired| \\
\path|runtime-upgrade| & An incompatible spec, metadata, interface, or fixture change fails startup pending a new policy & \path|runtime_upgrade_incompatible| \\
\end{longtable}
}
...
\item
  at least \path|max(3, 2 * M_gate)| active miners from at least two independent
  implementations, a policy-pinned panel size of at least \path|2 * M_gate|, and enough
  retained rolling assignments for at least \path|2 * M_gate| miners to satisfy every
  observation minimum;
\item
  ...
  \path|umi-publisher-control-disclosure/1| objects whose hashes match the signed
...
\item
  valid public \path|umi-activation-drill/1| reports for every drill in the table above;
\item
  published calibration results for batch size, deadlines, quality floor, strata,
...
Signed-request maximum age and allowed clock skew (\path|btauth_max_age_seconds|, \path|btauth_allowed_skew_seconds|) & 360 seconds and 2 seconds \\
...
Source-monitor bootstrap profile & \path|umi-source-bootstrap/1|; 10,000 signer-cluster replicates per validator, miner root, source, and window \\
...
Except for the exact temporary \path|bootstrap_service_binary| path incorporated in
Section 10.3, under \path|translation_weights_active: false| a conforming shadow
validator follows the same rules through deterministic weight build, emits no UMI
translation-weight commit, and publishes the \path|calibration_no_weight| bundle
defined in Section 10.3. A bootstrap service validator follows the shared
supersession\textquotesingle s separate conformance and chain-receipt rules and MUST NOT present
