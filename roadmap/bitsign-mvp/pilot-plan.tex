...
\author{}
\date{Draft v0.9, 29 August 2026}
...
\begin{itemize}[nosep]
  ...
  \item Camera capture, clip review, reshoot, upload progress, cancel, and clear error
        states.
  \item Returned English text, optional on-device speech, retry, discard, and an
        optional local text edit.
  \item A settings screen showing app and model versions, consent status when
        applicable, and an in-app account-deletion action with status tracking.
  \item VoiceOver labels, Dynamic Type, sufficient contrast, and visual equivalents
        for every status communicated by sound.
\end{itemize}

Outside a separately consented pilot or research protocol, ordinary Communicate offers
only a local text edit. It remains in memory for the current app session and is
discarded with the result. There is no ordinary rating or feedback endpoint. The
closed pilot uses the evidence protocol in Section~\ref{sec:pilotdata}; ordinary
Communicate use cannot be converted into pilot evidence after capture.
...
administration. The pilot collects only the evaluation evidence defined in its
separate consent and never training data. Any later research study uses another
informed-consent protocol and recruits separately from the pilot.
...
Recruit 8 to 12 Deaf or hard-of-hearing adult ASL signers, age 18 or older, for a
structured usability study. Minor participation is outside this protocol.
...
\subsection{Pilot evidence and consent}
\label{sec:pilotdata}

Reviewers need the source clip and exact returned output, so the pilot uses a data
class separate from ordinary Communicate and from training contributions. Enrollment
consent covers the three tasks, reviewer and incident access, withdrawal, aggregate
publication, and the retention schedule. Before each task upload, the participant
confirms pilot-evidence use; declining skips that task.

The app uploads with \code{purpose=pilot\_evaluation} to a dedicated private bucket.
The restricted record stores an opaque participant code, task id, clip digest and
object key, exact output, model and configuration versions, ratings, review results,
and timestamps. An ordinary Communicate upload cannot be copied or promoted into this
store. Pilot evidence is excluded from training and the frozen model evaluation.

Raw clips and linked exact outputs are deleted by the earlier of 30 days after final
...
collection. The code key is held separately, and only aggregate, de-identified counts
may remain in the report. Two named Deaf ASL-fluent reviewers
receive time-limited access, and every access is audited. A retention miss,
unauthorized access, or loss of withdrawal control pauses the pilot.

Pilot capture and upload are foreground-only and have no offline queue. A protected,
backup-excluded local clip is deleted once its upload is accepted or when the task is
declined, cancelled, withdrawn, or abandoned. Reviewer downloads follow the same
local protection and are removed on submission, expiry, permission revocation, or
launch cleanup.

The access audit contains actor id, object or review id, time, action, and outcome, but
no clip or transcript content. It is retained through the later of 90 days after the
final session and closure of a related incident, subject to an absolute 365-day cap
from the event. Withdrawal does not erase this minimal security record, and the
enrollment consent states that exception. The app
uses an inactive-scene privacy cover but cannot prevent a reviewer from taking an iOS
system screenshot; reviewer terms prohibit screenshots and the consent names this
...
protocol follows Section~\ref{sec:pilotdata}; its clips remain excluded from training
and the frozen model evaluation.
...
the model, or stop. \\
3. Pilot candidate & Accessible TestFlight build connected to the live inference
service, with isolated participant and reviewer evidence flows. & Proceed when R3
through R5 pass on this exact build. \\
...
Ordinary Communicate and pilot-evidence clips use foreground upload. Contribute clips
use their separate consented store and may use background upload. A GPU runtime is used
when the approved model and measured threshold require it; a CPU deployment must clear
the same release gates.

Routine clips are deleted after successful inference and within 24 hours after
cancellation, timeout, or failure. Server tests cover API storage, runtime caches,
queues, provider logs, and crash analytics, or record that a layer retains no clip
material. Separate iOS tests cover temporary and queued files, backup exclusion,
Keychain state, and in-memory results. No server test claims to delete a device file.
Pilot evidence follows the 30-day and 90-day limits in
Section~\ref{sec:pilotdata} and has a separate retention drill.
