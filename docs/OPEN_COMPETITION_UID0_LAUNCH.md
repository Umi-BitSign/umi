...
The approved launch uses `umi-open-competition-policy/3` paired with
`umi-competition-suite/2`. Version 3 retains version 2's scoring profile and adds
the verified burn destination for the unallocated model share. It uses authentic
single references, fingerspelling CER weighted 3/13 and continuous-signing WER
weighted 10/13. Michael's supplied
