...
/// Proof from chain initialization that a caller-supplied finalized database was selected.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct VerifiedDatabaseBootstrap {
    /// Finalized block number selected from the database.
    pub block_number: u64,
    /// Finalized block hash selected from the database.
...
impl LightClient {
    ...
    pub fn relay_chain<'a>(
        ...
        Self::relay_chain_inner(chain_config, "").map(|(client, rpc, _)| (client, rpc))
    }

    /// Given a chain spec and an explicitly supplied smoldot finalized database, establish a
    /// connection to a relay chain.
    ///
    /// UMI uses this narrow extension to start from a separately validated GRANDPA checkpoint.
    pub fn relay_chain_with_database<'a>(
        chain_config: impl Into<ChainConfig<'a>>,
        database_content: &str,
        expected_block_number: u64,
        expected_block_hash: [u8; 32],
    ) -> Result<(Self, LightClientRpc, VerifiedDatabaseBootstrap), LightClientError> {
        let (client, rpc, bootstrap) = Self::relay_chain_inner(chain_config, database_content)?;
        if bootstrap
            != Some(VerifiedDatabaseBootstrap {
                block_number: expected_block_number,
                block_hash: expected_block_hash,
            })
        {
            return Err(LightClientError::AddChainError(
                "caller-supplied finalized database was not selected at the expected block"
                    .to_owned(),
            ));
        }
        Ok((client, rpc, bootstrap.expect("exact Some value checked above")))
    }

    fn relay_chain_inner<'a>(
        chain_config: impl Into<ChainConfig<'a>>,
        database_content: &str,
    ) -> Result<
        (
            Self,
            LightClientRpc,
            Option<VerifiedDatabaseBootstrap>,
        ),
        LightClientError,
    > {
        ...
        let bootstrap = added_chain.used_database_chain_information.then_some(
            VerifiedDatabaseBootstrap {
                block_number: added_chain.initial_finalized_block_number,
                block_hash: added_chain.initial_finalized_block_hash,
            },
        );
        ...
        Ok((light_client, light_client_rpc, bootstrap))
    }
