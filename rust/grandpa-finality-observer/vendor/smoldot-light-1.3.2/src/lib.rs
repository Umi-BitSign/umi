...
pub struct AddChainSuccess<TPlat: PlatformRef> {
    ...
    /// Whether the initial finalized chain information came from `database_content`.
    pub used_database_chain_information: bool,

    /// Initial finalized block number selected while adding the chain.
    pub initial_finalized_block_number: u64,

    /// Initial finalized block hash selected while adding the chain.
    pub initial_finalized_block_hash: [u8; 32],
}
    ...
    pub fn add_chain(
        ...
        let (chain_information, used_database_chain_information, known_nodes, runtime_code_hint) = {
            ...
            match (genesis_chain_information, checkpoint, database) {
                ...
                // UMI patch: permit a caller-supplied, genesis-matched finalized database to be
                // the explicit bootstrap when the chain specification has no legacy
                // `lightSyncState`. Finney publishes its trusted GRANDPA checkpoint in the
                // `grandpaWarpSyncCheckpoint` extension instead; UMI validates that extension
                // and constructs the database chain information before calling `add_chain`.
                (
                    _,
                    None,
                    Some(database::DatabaseContent {
        ...
        let initial_finalized_block_number = chain_information
            .as_ref()
            .map(|information| information.as_ref().finalized_block_header.number)
            .unwrap_or(0);
        let initial_finalized_block_hash = chain_information
            .as_ref()
            .map(|information| {
                information
                    .as_ref()
